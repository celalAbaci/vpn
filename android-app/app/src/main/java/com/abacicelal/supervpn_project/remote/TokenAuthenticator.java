...
public class TokenAuthenticator implements Authenticator {
    ...
    public Request authenticate(@Nullable Route route, @NonNull Response response) throws IOException {
        Log.d(TAG, "authenticate: 401 (Yetkisiz) hatası alındı. Token yenileme deneniyor...");

        // 1. Mevcut (eski) refresh token'ı al
        String refreshToken = RetrofitClient.getRefreshToken(context);

        if (refreshToken == null) {
            Log.e(TAG, "authenticate: Refresh token bulunamadı. Yenileme yapılamıyor.");
        ...
        try {
            ...
            if (refreshResponse.isSuccessful() && refreshResponse.body() != null) {
                ...
                Log.i(TAG, "authenticate: Token başarıyla yenilendi.");
                ...
                Log.e(TAG, "authenticate: Refresh token ile yenileme başarısız oldu. Kod: " + refreshResponse.code());
                redirectToLogin();
                return null; // Yenileme başarısız
            }
        } catch (IOException e) {
            Log.e(TAG, "authenticate: Token yenileme sırasında ağ hatası.", e);
            return null; // Yenileme başarısız
        }
    }
